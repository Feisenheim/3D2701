...
typedef struct t_SecurityTickData {
  ...
  int32_t syl2;                     //市盈率2,2位小数.股票：价格/本年每股利润,债券：到期收益率,基金：每百份的IOPV 或净值 权证：溢价率
  ...
  //
  // 深交所： 产品所处的交易阶段代码
  // 第0位： S=启动（开市前）, O=开盘集合竞价, T=连续竞价, B=休市, C=收盘集合竞价,
...
typedef struct t_FuturesTickData {
  ...
  uint32_t pre_close;         //昨收盘价
  uint32_t pre_settle_price;  //昨结算价
  uint32_t open;              //开盘价
  uint32_t high;              //最高价
  uint32_t low;               //最低价
  uint32_t match;             //最新价
  ...
  uint32_t close;             //今收盘
  uint32_t settle_price;      //今结算价
  uint32_t high_limited;      //涨停价
  uint32_t low_limited;       //跌停价
  int32_t pre_delta;          //昨虚实度
  int32_t curr_delta;         //今虚实度
  uint32_t ask_price[5];      //申卖价
  uint32_t ask_vol[5];        //申卖量
  uint32_t bid_price[5];      //申买价
...
typedef struct t_OptionsTickData {
  char symbol[24];              // 证券代码（带交易所代码）
  int32_t data_timestamp;       // 时间戳 HHMMSSmmm,如果期权合约的产品代码为“00000000”，则表示行情时间
  int64_t pre_settle_price;     // 昨日结算价 4 decimal places
  int64_t settle_price;         // 今日结算价  4 decimal places
  int64_t open;                 // 开盘价. 如果期权合约的产品代码为“00000000”,则表示收盘标志,111111表示收盘
  int64_t high;                 // 最高价 
  int64_t low;                  // 最低价
...
typedef struct t_SecurityKdata {
  ...
  int32_t open;             // 开盘价      单位：1/100分,比如1表示0.0001元
  int32_t high;             // 最高价      单位：1/100分
  int32_t low;              // 最低价      单位：1/100分
  int32_t close;            // 收盘价      单位：1/100分
...
typedef struct t_CodeInfo {
  ...
  // 只对期权有效
  uint32_t margin_unit;         // 单位保证金	N16(2)	当日持有一张合约所需要的保证金数量，精确到分
  int32_t margin_ratio_param1;  // 保证金计算比例参数一	N6(2)	保证金计算参数，单位：%
  int32_t margin_ratio_param2;  // 保证金计算比例参数二	N6(2)	保证金计算参数，单位：%
} CodeInfo;
...
typedef struct t_TickByTickEntrust {
  ...
  char ord_type;				    // 订单类别: '1': 市价; '2': 限价; 'U': 本方最优
} TickByTickEntrust;
